...
vgood = zeros(ny,nd);  %continue repaying
vbad = zeros(ny,1); 
...
dp = zeros(ny,nd); %debt policy function (expressed in indices)  
dp1 = dp;
q = ones(ny,nd)/(1+rstar); %q is price of debt; it is a function of  (y_t, d_{t+1}) 
nondeflong = zeros(ny,nd);
...
% ss.EdgeColor = 'interp';
% axis tight
% xlabel('y state','interpreter','latex')
% ylabel('b policy','interpreter','latex')
% zlabel('Debt price')
% %colorbar;
% ax = gca;
% ax.GridAlpha = 0.6;
...
fname = sprintf('one_%dy_%dd.mat', ny,nd);
